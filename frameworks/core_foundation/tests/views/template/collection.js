...
test("should re-render when the content object's nested objects change", function() {
  ...
  var view = SC.TemplateView.create({
    template: SC.Handlebars.compile('{{#collection TemplateTests.RerenderNestedTBodyTest tagName="tbody"}}{{#with content}}<td>{{title}}</td><td>{{user.name}}</td>{{/with}}{{/collection}}')
  });
  ...
  equals(view.$('td:eq(0)').text(), "SproutCore");
